...
export type LabelKind = 'district' | 'mixed' | 'landmark' | 'venue' | 'label' | 'busker' | 'home' | 'street';
...
export class LabelLayer {
  ...
  update(camera: THREE.PerspectiveCamera, target: THREE.Vector3, width: number, height: number, walking = false): void {
    ...
    const walkRange: Record<LabelKind, number> = { district: 150, mixed: 0, landmark: 70, venue: 45, label: 160, busker: 22, home: 35, street: 30 };
